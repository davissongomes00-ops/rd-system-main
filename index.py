# ---- ROTA 2: PÁGINA EXCLUSIVA DE CALCULADORA & LOGÍSTICA ----
...
async def read_checkout():
    ...
    <head>
        ...
        <title>RD Logistics | Dimensionamento</title>
        ...
        <style>
            ...
            .container {{ display: grid; grid-template-columns: 1.8fr 1.2fr; gap: 40px; max-width: 1200px; margin: auto; padding: 20px; }}
            .software-card, .sidebar-summary {{ background: #0a0a0a; border: 1px solid #222; border-radius: 12px; padding: 30px; }}
            input[type="number"] {{ width: 100%; padding: 14px; background: #111; border: 1px solid #333; color: white; border-radius: 6px; font-size: 1.2rem; box-sizing: border-box; margin-bottom: 20px; }}
            .budget-table {{ width: 100%; border-collapse: collapse; margin-top: 20px; }}
            .budget-table th {{ text-align: left; padding: 12px; color: var(--nexus-blue); font-family: 'Orbitron'; border-bottom: 2px solid #222; }}
            .budget-table td {{ padding: 12px; border-bottom: 1px solid #111; font-size: 0.95rem; }}
            .total-row {{ font-family: 'Orbitron'; font-weight: bold; color: var(--rd-red); background: #050505; }}
            .btn-whatsapp-calc {{ width: 100%; background: #25D366; color: white; border: none; padding: 16px; border-radius: 6px; font-family: 'Orbitron'; font-weight: bold; cursor: pointer; font-size: 1rem; margin-top: 20px; }}
            
            /* Animação do camião */
            .highway-container {{ position: relative; width: 100%; height: 40px; background: #111; border-radius: 6px; margin-top: 30px; overflow: hidden; border-bottom: 2px dashed #333; }}
            .delivery-truck {{ position: absolute; bottom: 5px; left: -100px; width: 60px; height: 25px; animation: driveTruck 8s infinite linear; }}
            .truck-body {{ width: 42px; height: 20px; background: #fff; position: absolute; border-radius: 3px 0 0 3px; }}
            .truck-cabin {{ width: 16px; height: 16px; background: var(--nexus-blue); position: absolute; right: 0; bottom: 5px; border-radius: 0 5px 3px 0; }}
            .truck-cabin::after {{ content: ''; position: absolute; top: 3px; right: 2px; width: 7px; height: 8px; background: #000; border-radius: 0 4px 0 0; }}
...
# ---- ROTA 3: PÁGINA DE PAGAMENTO (CHECKOUT INTEGRADO OTIMIZADO) ----
...
async def read_pagamento():
    ...
    <head>
        ...
        <style>
            :root {{ --rd-red: #FF0000; --dark-bg: #000000; --nexus-blue: #00d2ff; }}
            body {{ margin: 0; background: var(--dark-bg); color: white; font-family: 'Urbanist', sans-serif; padding-top: 110px; }}
            ...
            input[type="text"] {{ width: 100%; padding: 14px; margin-bottom: 20px; background: #111; border: 1px solid #333; color: white; border-radius: 6px; font-size: 1rem; box-sizing: border-box; }}
    ...
    <body>
        <div class="pay-container">
            ...
            <div style="background:#111; padding:15px; border-radius:6px; margin-bottom:25px; display:flex; justify-content:space-between; align-items:center;">
                ...
                <strong id="valorTotalLiquidar" style="font-family:'Orbitron'; font-size:1.5rem; color:var(--nexus-blue);">0.00€</strong>
            </div>
